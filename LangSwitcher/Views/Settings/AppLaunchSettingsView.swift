...
struct AppLaunchSettingsView: View {
    ...
    var hasIncomplete: Bool { settings.activeProfile.payload.appLaunchShortcuts.contains { $0.displayString.isEmpty || $0.bundleIdentifier.isEmpty } }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack {
                ...
                Spacer()
                Toggle("", isOn: $settings.isAppLaunchEnabled) // 🌟 전역 변수 유지
                    .toggleStyle(.switch)
                    .labelsHidden()
                    .controlSize(.small)
            ...
            VStack(alignment: .leading, spacing: 15) {
                HStack {
                    ...
                    Spacer()
                    Button(action: { if !hasIncomplete { settings.activeProfile.payload.appLaunchShortcuts.append(AppLaunchShortcut(keyCode: 0, modifierFlags: 0, displayString: "", bundleIdentifier: "", appName: "")) } }) {
            ...
            .opacity(settings.isAppLaunchEnabled ? 1.0 : 0.5)
            .disabled(!settings.isAppLaunchEnabled)
